...
class Environment:
	def __init__(self, size, initState = None): 
		...
		if (initState == None):
			...
			raise Exception("Provided initial state is not a valid state for this environment")
	...
	def calculateAllThreats(self):
		...
		for queen1 in range(self.size):
			for queen2 in range(queen1+1, self.size):
				if (queen1 != queen2):
					threat = self.__areQueensThreatening((queen1, self.queens[queen1]), (queen2, self.queens[queen2]) )
					if (threat):
		...
		for column in range(self.size):
			for row in range(self.size):
				if(self.queens[column] != row):
					nextPairs = self.__getThreatenedQueensForMovement((column, row))
	...
	def __areQueensThreatening(self, queen1, queen2):
		...
		if (queen1Row == queen2Row): # same row
			return True
		if (abs(queen1Col - queen2Col) == abs(queen1Row - queen2Row)): # same diagonal
			return True
		return False
	...
	def __getThreatenedQueensForMovement(self, movement):
		...
		for pair in newThreatened:
			if (pair[0] == queenColumn or pair[1] == queenColumn):
				newThreatened.remove(pair)
		# 2. Recalculate the threat from that queen to the other n-1 queens and push them twice, as (q1, q2) and (q2, q1).
		for otherQueen in range(self.size):
			if (otherQueen != queenColumn):
				threat = self.__areQueensThreatening(movement, (otherQueen, self.queens[otherQueen]))
				if (threat):
	...
	def print(self):
		...
		size = self.size
		matrix = [[icon(row, column) for row in range(size)] for column in range(size)]
		for i in range(size):
			for j in range(size):
